...
interface ReplaceObjectIdOptions {
  indexName?: string;
  dryRun: boolean;
}

export async function replaceObjectIds(
  options: ReplaceObjectIdOptions
): Promise<void> {
  ...
  console.log(
    `🔍 ${options.dryRun ? "DRY RUN" : "EXECUTING"} - Replace objectID action`
  );
  ...
  if (!options.dryRun) {
    const confirmation = await promptUser(
      `⚠️  This will modify objectID values in index "${indexName}". This is a destructive operation that cannot be undone.\n` +
  ...
  try {
    ...
    while (true) {
      ...
      for (const record of response.hits as unknown[]) {
        ...
        const newObjectId = record.title.toLowerCase();
